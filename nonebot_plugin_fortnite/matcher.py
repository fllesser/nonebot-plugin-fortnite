...
from nonebot_plugin_alconna import on_alconna, AlconnaMatcher, Match
...
battle_pass = on_alconna(Alconna(timewindow_prefix, "季卡", name_args))

stats = on_alconna(Alconna(timewindow_prefix, "战绩", name_args))
...
async def _():
    await shop.send(
        await UniMessage(
            ...
            + Text("https://www.fortnite.com/item-shop?lang=zh-Hans")
        ).export()
    )
...
async def _():
    ...
    try:
        ...
        await update_vb.send(
            await UniMessage(Text("手动更新商城成功") + Image(path=file)).export()
        )
...
async def _():
    ...
    try:
        ...
        await update_vb.send(
            await UniMessage(Text("手动更新vb图成功") + Image(path=file)).export()
        )
    except Exception as e:
